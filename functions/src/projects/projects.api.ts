...
import { createBoard, createGoal, createProject, deactivateProject, editProject, createTask, editTask, editBoard } from "./projects.service";
...
app.post("/api/v1/editBoard", async (req, res) => {
  try {
    const body = req.body;
    const rawBoardData = body?.boardData;
    const goalId = body?.goalId;
...
app.post("/api/v1/editTask", async (req, res) => {
  try {
    const body = req.body;
    const rawTaskData = body?.taskData;
    const boardId = body?.boardId;
